...
if(isset($_POST['export']))
  ...
  $sort=mysql_real_escape_string($_POST['sort']);
		...
		$filename = "$sort Clearance"; //your file name
		...
		*/
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'Clearance Report');		
		$objPHPExcel->getActiveSheet()->setCellValue('A2', '('.$sort.')');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('H4', 'cleared');
		$objPHPExcel->getActiveSheet()->setCellValue('I4', 'date_clearance');
...
if ($sort=='Cleared'){
	$sql="SELECT c.id, c.emp_no, e.lastname, e.firstname, e.middlename, e.position, e.date_hired, c.date_clearance, c.cleared FROM tblclearance c INNER JOIN tblemployees e ON c.emp_no = e.emp_no WHERE c.cleared = 1 Order By e.lastname";
}
elseif ($sort=='NotCleared'){
	$sql="SELECT c.id, c.emp_no, e.lastname, e.firstname, e.middlename, e.position, e.date_hired, c.date_clearance, c.cleared FROM tblclearance c INNER JOIN tblemployees e ON c.emp_no = e.emp_no WHERE c.cleared = 0 Order By e.lastname";
}
else{
	$sql="SELECT c.id, c.emp_no, e.lastname, e.firstname, e.middlename, e.position, e.date_hired, c.date_clearance, c.cleared FROM tblclearance c INNER JOIN tblemployees e ON c.emp_no = e.emp_no Order By e.lastname";
...
$qry=mysql_query($sql);	
 ...
 while($d= mysql_fetch_array($qry)){
	if ($d['cleared']==0){
		...
		$cleared = "Yes";
		$clearancedate = $d['date_clearance'];
	}
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('H'.$n, $cleared);
 $objPHPExcel->getActiveSheet()->setCellValue('I'.$n, $clearancedate);
